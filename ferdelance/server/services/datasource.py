...
from ferdelance_shared.schemas import DataSource, Feature
...
class DataSourceService(DBSessionService):
    ...
    def create_or_update_datasource(self, client_id: str, ds: DataSource) -> ClientDataSource:
        dt_now = datetime.now()
    ...
    def create_or_update_feature(self, ds_id: str, f: Feature, remove: bool = False, commit: bool = True) -> ClientFeature:
        dt_now = datetime.now()
    ...
    def get_datasource_ids_by_client_id(self, client_id: str) -> list[str]:
        return self.db.query(ClientDataSource, ClientDataSource.datasource_id).filter(ClientDataSource.client_id == client_id).all()
    ...
    def get_client_id_by_datasource_id(self, ds_id: str) -> str:
        return self.db.query(ClientDataSource, ClientDataSource.client_id).filter(ClientDataSource.datasource_id == ds_id, ClientDataSource.removed == False).first()
