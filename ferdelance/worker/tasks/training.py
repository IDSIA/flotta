...
from ferdelance.worker.jobs.services import TrainingJobService
...
def training(self: GenericTask, raw_args: dict[str, Any]) -> None:
    task_id: str = str(self.request.id)
    args = TaskArguments(**raw_args)

    try:
        logging.info(f"worker: beginning training task={task_id}")
